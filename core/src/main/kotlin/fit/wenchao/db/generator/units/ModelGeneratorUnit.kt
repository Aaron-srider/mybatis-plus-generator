...
import fit.wenchao.db.sourceFile.fromUnderScore
import java.util.*
...
class ModelGeneratorUnit(generatorContext: GeneratorContext) : GenericGeneratorUnit(generatorContext) {
    ...
    override fun doGenerate(partialMap: HashMap<String, Any>, newInfoRegistor: NewInfoRegistor) {
        ...
        tables.forEach { table ->
            val modelClazzName = JavaClassNameImpl.fromLowerUnderScore(modelPackage, table.name, "PO")
            ...
            var code = if (language == Lang.KOTLIN) {
                kotlinWriter()
                    ...
                    .datal().classl(modelClazzName.getClazzName()).primaryConstructor { jc: KotlinCodeWriter ->
                ...
                javaWriter()
                    ...
                    .atl("TableName(\"`" + table.name + "`\")")
                    .publicl().classl(modelClazzName.getClazzName()).implementsl("Serializable")
